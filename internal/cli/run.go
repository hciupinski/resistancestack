...
func printUsage(out io.Writer) {
	...
	fmt.Fprintln(out, "")
	fmt.Fprintln(out, "Config-backed commands also accept:")
	fmt.Fprintln(out, "  --config <path>                     Base configuration file (default: resistack.yaml)")
	fmt.Fprintln(out, "  --env <name>                       Environment overlay name, resolved as resistack.<name>.yaml")
	fmt.Fprintln(out, "")
